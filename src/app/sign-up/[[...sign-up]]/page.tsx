...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (createError) {
      setError(createError.message || "Une erreur est survenue");
      setLoading(false);
      return;
    }
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    if (verifyError) {
      setError(verifyError.message || "Code de vérification invalide");
      setLoading(false);
      return;
    }
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {!verifying ? (
              ...
              >
                <div className="mb-8">
                  <h1 className="text-2xl font-semibold text-white tracking-tight">Start building</h1>
                  <p className="mt-1.5 text-sm text-white/45">500 requêtes gratuites, sans carte bancaire</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-white/60 mb-1.5">Prénom</label>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-white/30">Minimum 8 caractères</p>
                  </div>
                ...
                <p className="mt-8 text-center text-sm text-white/40">
                  Déjà un compte?{" "}
                  <Link href="/sign-in" className="text-teal-400 hover:text-teal-300 font-medium transition-colors">
                    Se connecter
                  </Link>
                </p>
              </motion.div>
              ...
              >
                <div className="mb-8 text-center">
                  ...
                  <h1 className="text-2xl font-semibold text-white tracking-tight">Vérifiez votre email</h1>
                  <p className="mt-2 text-sm text-white/45">
                    Code envoyé à <span className="text-white">{email}</span>
                  </p>
                </div>

                <form onSubmit={handleVerify} className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-white/60 mb-1.5">Code de vérification</label>
                    <input
                      type="text"
                      value={code}
                      ...
                      required
                      maxLength={6}
                      className="w-full px-4 py-3 rounded-lg bg-white/[0.04] border border-white/10 text-white text-center text-xl tracking-[0.4em] font-mono placeholder-white/20 outline-none focus:border-teal-500/40 transition-all"
                    />
                  </div>
